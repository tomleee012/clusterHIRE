...
float val2(float** P_t, float* Eps_M, short int num_celltypes, short int num_CpGsites, short int num_cancer_subtypes,
	float** Ometh, float**** E_sigma, float*** E_mu, float** E_Z, short int n) {
	short int K, M, Q;
	K = num_celltypes;
	M = num_CpGsites;
...
void Update_P(float** P_t, float* Eps_M, short int num_celltypes, short int num_samples, short int num_CpGsites, short int num_cancer_subtypes,
	float** Ometh, float**** E_sigma, float*** E_mu, float** E_Z) {
...
void EstepI(short int* Labels, float** P_t, float** Mu, float*** Beta, float*** Gamma, float* Eps_M,
	...
	short int num_covariates, float** Ometh, float** Covariates, float**** E_sigma, float*** E_mu, float** E_Z) {
...
void MstepI(float* Pi_t, float** P_t, float** Mu, float*** Beta, float*** Gamma, float* Eps_M, float** Eps_MK,
	...
	float** Ometh, float** Covariates, float**** E_sigma, float*** E_mu, float** E_Z) {
	short int K, N, M, Q, P;
	K = num_celltypes;
	N = num_samples;
	M = num_CpGsites;
		...
		for (k = 0; k < K; k++) {
			...
			for (int n = 0; n < N; n++) {
				for (int q = 0; q < Q; q++) {
					...
					s1 += (E_mu[n][m][k] - s2 - Gamma[m][k][q]) * E_Z[n][q];
				}
			}
			...
			for (n = 0; n < N; n++) {
				...
				for (q = 0; q < Q; q++) {
					s3 += (pow(E_mu[n][m][k] - Mu[m][k] - s1 - Gamma[m][k][q], 2) + E_sigma[n][m][k][k]) * E_Z[n][q];
				}
			}
			Eps_MK[m][k] = s3 / N;
		}
		...
		for (k = 0; k < K; k++) {
			...
			for (q = 1; q < Q; q++) {
				...
				for (n = 0; n < N; n++) {
					...
					//}
					//s4 -= Gamma[m][k][q];
					s1 += (E_mu[n][m][k] - Mu[m][k] - s2 - s4) * E_Z[n][q];
					s3 += E_Z[n][q];
				}
				Gamma[m][k][q] = s1 / s3;
			}
			...
			for (p = 0; p < P; p++) {
				...
				for (n = 0; n < N; n++) {
					...
					s2 -= Beta[m][k][p] * Covariates[n][p];
					for (q = 0; q < Q; q++) {
						s1 += (E_mu[n][m][k] - Mu[m][k] - s2 - Gamma[m][k][q]) * Covariates[n][p] * E_Z[n][q];
					}
